"""Tabular-SQL execution — run the data query engine over a table the
orchestrator already chose, and turn it into an exact answer.

Routing lives in the orchestrator (main.py): rewrite → retrieve candidate
tables → ONE grounded route decision (CLARIFY / RAG / SQL). When the route is
SQL, the orchestrator calls run_sql() here with the chosen candidate table.

run_sql flow (any failure → None, so the orchestrator falls back to RAG):
  1. load_tables   — rebuild the chosen document's table(s) from row_data
                     (ACL-filtered to the user's visible rows).
  2. SQL agent loop — the model writes SQL, runs it read-only in in-memory
  ...
  3. format        — render the exact result as a context block for the
                     generator to phrase the final answer around.

Returning None means "no confident SQL answer" — the orchestrator runs RAG.
This is the safe default: a missed aggregation degrades to today's behavior,
never to a crash.

ACL: load_tables rebuilds the table from ONLY the user's visible rows, so a
...
from backend.generation.llm import sql_agent_step
...
async def run_sql(
    ...
    """Run the SQL agent over the table the orchestrator already chose (`chosen`,
    one candidate from retrieve_tables). Returns {context, sources, sql,
    table_name, result} on success, or None to fall back to RAG.

    ACL: load_tables filters to the user's visible rows, so the aggregate can
    never expose data the user couldn't retrieve normally; a user with no visible
    rows gets None → RAG."""
    tables = await load_tables(
        session, chosen["document_id"],
        user_email=user_email, user_groups=user_groups or [],
    )
    if not tables:
        # row_data missing (pre-Phase-1 doc not re-ingested) or no visible rows.
        logger.info("run_sql: doc %s not rebuildable / no visible rows -> rag", chosen["document_id"])
        return None
    ...
    try:
        ...
        logger.warning("run_sql: build_connection failed (%s) -> rag", exc)
        return None
    try:
    ...
    if sql is None or not result or not result["rows"]:
        logger.info("run_sql: agent produced no usable answer -> rag")
        return None

    logger.info("run_sql: answered table=%s rows=%d", chosen["table_name"], result["row_count"])
    return {
